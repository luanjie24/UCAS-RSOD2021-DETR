...
Original file is located at
    https://colab.research.google.com/drive/1W-bYxDQgNwtJ7ILZohRRjkDjJAjvlzWo
...
class DETRdemo(nn.Module):
    ...
    * fc bbox predictor (instead of MLP)
    ...
    def __init__(self, num_classes, hidden_dim=256, nheads=8,
        ...
        # 就是原文图2的backbone，用于将输入图片转为向量（提取图片特征）ResNet-50资料：https://blog.csdn.net/weixin_44791964/article/details/102790260
        self.backbone = resnet50()
        # 删除了ResNet-50最后的全连接层
        del self.backbone.fc

        # create conversion layer
        # 二维卷积 输入通道数2048，输出通道数hidden_dim，就是论文里的d，卷积核1*1，步长默认1
        ...
        # 用单个全连接层替代baseline的DETR的三个全连接的MLP（FFN）
    ...
    def forward(self, inputs):
        # propagate inputs through ResNet-50 up to avg-pool layer （平均池化）
...
detr.load_state_dict(state_dict) # 直接下载并读取现有模型的参数作为预训练模型
detr.eval();
